...
void ActivateTera(enum BattlerId battler)
{
    ...
    if (TryBattleFormChange(gBattlerAttacker, FORM_CHANGE_BATTLE_TERASTALLIZATION, GetMonAbility(GetBattlerMon(gBattlerAttacker))))
...
bool32 CanTerastallize(enum BattlerId battler)
{
    ...
    if (GetBattlerTrainer(battler) == B_TRAINER_PLAYER
     && !GetMonData(GetBattlerMon(battler), MON_DATA_TERA_UNLOCKED))
        return FALSE;
