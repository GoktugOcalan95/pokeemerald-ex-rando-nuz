...
static enum CanMoveBeLearned CanTeachMove(struct Pokemon *mon, enum Move move)
{
    ...
    else if (!CanPlayerLearnTeachableMove(GetMonData(mon, MON_DATA_SPECIES_OR_EGG), move))
...
static void CB2_SelectBagItemToGive(void)
{
    if (CurrentBattlePyramidLocation() == PYRAMID_LOCATION_NONE)
        GoToBagMenu(ITEMMENULOCATION_PARTY, BAG_POCKETS_COUNT, CB2_GiveHoldItem);
    else
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    sInitialLevel = GetMonData(mon, MON_DATA_LEVEL);
    if (!((AreLevelCapsEnabled() || B_RARE_CANDY_CAP) && sInitialLevel >= GetCurrentLevelCap()))
    {
